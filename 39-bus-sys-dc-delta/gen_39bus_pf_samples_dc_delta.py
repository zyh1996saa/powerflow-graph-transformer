...
# ============================================================
DATAPATH = "/data2/zyh"
DATASET_ROOT = os.path.join(DATAPATH, "case39_samples_dc_delta")
TRAIN_SUBDIR = "train"
TEST_SUBDIR = "test"
...
GENERATOR_VERSION = "dc_delta_baseline_v1_runpp_truth_rundcpp_baseline"
...
def save_network_metadata(net, output_dir: str) -> Dict[str, Any]:
    metadata = {
        "network_info": {
            ...
            "dc_delta_convention": "H_ac saved as H_{idx}.npy; H_dc saved as H_dc_{idx}.npy; model learns H_ac-H_dc on bus-type target columns.",
        },
        "buses": {
            int(bus_idx): {
...
def _get_bus_result_value(res_bus, bus_id: int, key: str, default: float = 0.0) -> float:
    if res_bus is None or len(res_bus) <= 0 or bus_id not in res_bus.index:
        return float(default)
    try:
        value = float(res_bus.loc[bus_id].get(key, default))
        return value if np.isfinite(value) else float(default)
    except Exception:
        return float(default)


def build_ac_truth_and_y(net) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
    """Extract AC truth H and the external-order Ybus after a converged pp.runpp call."""
    ...
    for _, load in net.load.iterrows():
        if not bool(load.get("in_service", True)):
            continue
    ...
    for gen_idx, gen in net.gen.iterrows():
        if not bool(gen.get("in_service", True)):
            continue
    ...
    if len(net.ext_grid) > 0 and hasattr(net, "res_ext_grid") and len(net.res_ext_grid) > 0:
        for ext_idx, ext_grid in net.ext_grid.iterrows():
            if not bool(ext_grid.get("in_service", True)):
                continue
...
def build_dc_baseline_matrix(net, bus_ids: List[int], state_valid_mask: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
    """
    Build H_dc with the same six columns as H_ac.

    Pd/Qd: load input.
    Pg: PV uses input gen p_mw; SLACK uses DC res_ext_grid.p_mw; PQ remains zero.
    Qg: zero.
    Vm: PV/SLACK setpoint vm_pu; PQ flat 1.0.
    Va: DC res_bus.va_degree.
    """
    bus_map = {bus_id: pos for pos, bus_id in enumerate(bus_ids)}
    n_buses = len(bus_ids)
    H_dc = np.zeros((n_buses, 6), dtype=np.float64)
    H_dc[:, IDX_VM] = 1.0
    ...
    for _, load in net.load.iterrows():
        ...
        if bus not in bus_map:
            continue
        i = bus_map[bus]
        H_dc[i, IDX_PD] += float(load.get("p_mw", 0.0))
        H_dc[i, IDX_QD] += float(load.get("q_mvar", 0.0))
    ...
    for _, gen in net.gen.iterrows():
        ...
        if bus not in bus_map:
            continue
        i = bus_map[bus]
        H_dc[i, IDX_PG] += float(gen.get("p_mw", 0.0))
        H_dc[i, IDX_VM] = float(gen.get("vm_pu", H_dc[i, IDX_VM]))
    ...
    if len(net.ext_grid) > 0:
        for ext_idx, ext_grid in net.ext_grid.iterrows():
            ...
            if bus not in bus_map:
                continue
            i = bus_map[bus]
            H_dc[i, IDX_PG] += _get_result_value(getattr(net, "res_ext_grid", None), int(ext_idx), "p_mw")
            H_dc[i, IDX_VM] = float(ext_grid.get("vm_pu", H_dc[i, IDX_VM]))

    res_bus = getattr(net, "res_bus", None)
    missing_dc_va_bus_ids: List[int] = []
    for bus_id in bus_ids:
        pos = bus_map[bus_id]
        H_dc[pos, IDX_VA] = _get_bus_result_value(res_bus, bus_id, "va_degree", default=0.0)
        if bool(state_valid_mask[pos]) and (res_bus is None or bus_id not in res_bus.index):
            missing_dc_va_bus_ids.append(int(bus_id))

    H_dc[~state_valid_mask.astype(bool), :] = 0.0
    aux = {
        "dc_baseline_policy": {
            "Pd_Qd": "load input",
            "Pg": "PV input gen p_mw; SLACK rundcpp res_ext_grid.p_mw; PQ zero",
            "Qg": "zero",
            "Vm": "PV/SLACK vm_pu setpoint; PQ 1.0",
            "Va": "rundcpp res_bus.va_degree",
        },
        "missing_dc_va_bus_ids": missing_dc_va_bus_ids,
    }
    return H_dc, aux


def run_ac_dc_extract(net) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict[str, Any]]:
    ...
    H_ac, Y, ac_aux = build_ac_truth_and_y(net)
    bus_ids = [int(x) for x in ac_aux["bus_ids_sorted"]]
    state_valid_mask = np.asarray(ac_aux["state_valid_mask"], dtype=bool)

    pp.rundcpp(net, check_connectivity=True)
    H_dc, dc_aux = build_dc_baseline_matrix(net, bus_ids=bus_ids, state_valid_mask=state_valid_mask)
    aux = dict(ac_aux)
    aux.update(dc_aux)
    aux["h_dc_shape"] = list(H_dc.shape)
    aux["dc_converged"] = bool(getattr(net, "converged", True))
    return H_ac, H_dc, Y, aux
...
def build_one_sample(sample_idx: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # pp.runpp has already converged, but run_ac_dc_extract reruns it to keep the extraction self-contained.
        H_ac, H_dc, Y, aux = run_ac_dc_extract(net)
        sparse_y = csr_matrix(Y)
        target_delta = H_ac - H_dc
        sample_metadata = {
            ...
            "h_shape": list(H_ac.shape),
            "h_dc_shape": list(H_dc.shape),
            "delta_shape": list(target_delta.shape),
...
def main() -> None:
    ...
    print(f"DC-Delta 数据集已生成到: {root_dir}")
