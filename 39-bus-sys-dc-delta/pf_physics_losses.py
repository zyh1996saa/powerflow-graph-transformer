...
class MaskedMSELoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        err2 = ((pred - target) ** 2) * mask.to(pred.dtype)
        denom = mask.to(pred.dtype).sum().clamp_min(self.eps)
        return err2.sum() / denom


class BusTypeDeltaLoss(nn.Module):
    """MSE on normalized delta targets selected by bus type."""
    ...
    def forward(
        ...
        if pred_delta_norm.dim() == 2:
            pred_delta_norm = pred_delta_norm.unsqueeze(0)
            target_delta_norm = target_delta_norm.unsqueeze(0)
            state_valid_mask = state_valid_mask.unsqueeze(0)
        bsz, num_nodes, _ = pred_delta_norm.shape
        bt = bus_type[:num_nodes]
        total_loss = pred_delta_norm.new_tensor(0.0)
        total_weight = pred_delta_norm.new_tensor(0.0)
        ...
        def group_loss(name: str, node_mask_1d: torch.Tensor, cols: List[int], weight: float) -> None:
            ...
            if weight <= 0:
                stats[f"{name}_delta_mse"] = 0.0
                stats[f"{name}_delta_rmse"] = 0.0
                return
            group_mask = state_valid_mask & node_mask_1d.view(1, num_nodes)
            expanded = group_mask.unsqueeze(-1).expand(bsz, num_nodes, len(cols))
            denom = expanded.to(pred_delta_norm.dtype).sum().clamp_min(self.eps)
            mse = (
                ((pred_delta_norm[:, :, cols] - target_delta_norm[:, :, cols]) ** 2)
                * expanded.to(pred_delta_norm.dtype)
            ).sum() / denom
            total_loss = total_loss + weight * mse
            total_weight = total_weight + weight
            mse_value = float(mse.detach().item())
            stats[f"{name}_delta_mse"] = mse_value
            stats[f"{name}_delta_rmse"] = math.sqrt(max(mse_value, 0.0))
...
BusTypePowerFlowLoss = BusTypeDeltaLoss


def reconstruct_h_from_delta(
    H_dc_phys: torch.Tensor,
    delta_phys: torch.Tensor,
    bus_type: torch.Tensor,
    state_valid_mask: torch.Tensor,
) -> torch.Tensor:
    """
    Reconstruct H_pred = H_dc + delta only on bus-type target columns.
    Non-target columns remain exactly at the DC/given baseline value.
    """
    if H_dc_phys.dim() == 2:
        H_dc_phys = H_dc_phys.unsqueeze(0)
        delta_phys = delta_phys.unsqueeze(0)
        state_valid_mask = state_valid_mask.unsqueeze(0)
        squeeze = True
    else:
        squeeze = False
    ...
    target_mask = create_bus_type_target_mask(
        ...
        bus_type=bus_type,
        feat_dim=H_dc_phys.shape[-1],
    )
    H_pred = H_dc_phys.clone()
    H_pred = torch.where(target_mask, H_dc_phys + delta_phys, H_pred)
    H_pred = torch.where(state_valid_mask.unsqueeze(-1), H_pred, torch.zeros_like(H_pred))
    return H_pred.squeeze(0) if squeeze else H_pred
...
def delta_supervised_mse(
    ...
    target_mask = create_bus_type_target_mask(
        state_valid_mask=state_valid_mask,
        bus_type=bus_type,
        feat_dim=pred_delta_norm.shape[-1],
    )
    criterion = MaskedMSELoss(eps=eps)
    loss = criterion(pred_delta_norm, target_delta_norm, target_mask)
    stats = {
        "supervised_delta_mse": float(loss.detach().item()),
        "delta_rmse": math.sqrt(max(float(loss.detach().item()), 0.0)),
    }
    return loss, stats
