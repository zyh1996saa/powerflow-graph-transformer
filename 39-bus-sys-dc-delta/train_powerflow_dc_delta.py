...
from pf_topology_utils import StandardizationBundle, build_bus_type_vector, ensure_dir, get_network_base_mva, load_network_metadata
...
RUN_NAME = datetime.now().strftime("%Y%m%d_%H%M%S") + "_pf_dc_delta"
...
LOGGER = logging.getLogger("train_powerflow_dc_delta")
...
def build_standardizers(device: str) -> Optional[StandardizationBundle]:
    if not ENABLE_STANDARDIZATION:
        return None
    stats_path = Path(STANDARDIZATION_STATS_PATH)
    if not stats_path.exists():
        raise FileNotFoundError(f"标准化统计文件不存在: {stats_path}。请先运行 compute_train_hdc_delta_stats.py")
    return StandardizationBundle.from_npz(str(stats_path), device=torch.device(device))
...
def main() -> None:
    ...
    standardizers = build_standardizers(DEVICE)

    trainer = PFDeltaTrainer(
        model=model,
        bus_type=bus_type,
        ...
        model_cfg=MODEL_CFG,
        stats_path=STANDARDIZATION_STATS_PATH if ENABLE_STANDARDIZATION else None,
    )
