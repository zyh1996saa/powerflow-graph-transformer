...
class PowerFlowDCDeltaDataset(Dataset):
    """Dataset that returns AC truth H, DC baseline H_dc, and delta=H-H_dc."""
    ...
    def __init__(
        ...
        if not all_indices:
            raise RuntimeError(f"在目录 {self.data_dir} 中未发现完整样本文件：需要 H_*.npy、H_dc_*.npy、Y_*.npz、metadata_*.json")
        ...
        if self.cache_metadata and self.return_metadata:
            for sample_idx in self.indices:
                _, _, _, m_path = self._sample_paths(sample_idx)
    ...
    def _discover_indices(data_dir: Path) -> List[int]:
        ...
        for fname in os.listdir(data_dir):
            ...
            try:
                ...
                continue
            if (
                (data_dir / f"H_{idx}.npy").exists()
                and (data_dir / f"H_dc_{idx}.npy").exists()
                and (data_dir / f"Y_{idx}.npz").exists()
            ):
                indices.append(idx)
    ...
    def _sample_paths(self, sample_idx: int) -> Tuple[Path, Path, Path, Path]:
        return (
            self.data_dir / f"H_{sample_idx}.npy",
            self.data_dir / f"H_dc_{sample_idx}.npy",
    ...
    def __getitem__(self, item: int) -> Dict[str, Any]:
        ...
        h_path, h_dc_path, y_path, m_path = self._sample_paths(sample_idx)
        H_np = np.load(h_path, mmap_mode=self.h_mmap_mode)
        H_dc_np = np.load(h_dc_path, mmap_mode=self.h_mmap_mode)
        H_arr = np.asarray(H_np).copy()
        H_dc_arr = np.asarray(H_dc_np).copy()
        if H_arr.shape != H_dc_arr.shape:
            raise RuntimeError(f"样本 {sample_idx} 的 H 与 H_dc 形状不一致: {H_arr.shape} vs {H_dc_arr.shape}")

        H = torch.as_tensor(H_arr, dtype=self.h_dtype)
        H_dc = torch.as_tensor(H_dc_arr, dtype=self.h_dtype)
        delta = H - H_dc
        ...
        num_valid_nodes = self._infer_num_valid_nodes(H_arr, metadata)
        node_valid_mask = torch.zeros(H.shape[0], dtype=torch.bool)
        node_valid_mask[:num_valid_nodes] = True
        state_valid_mask = torch.as_tensor(self._infer_state_valid_mask(H_arr, metadata), dtype=torch.bool)
...
PowerFlowDataset = PowerFlowDCDeltaDataset
...
class PowerFlowCollator:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        H_dc_list = [item["H_dc"] for item in batch]
        delta_list = [item["delta"] for item in batch]
        ...
        if self.pad_to_max:
            ...
            H_dc = _pad_and_stack_dense_tensors(H_dc_list, pad_value=0.0)
            delta = _pad_and_stack_dense_tensors(delta_list, pad_value=0.0)
            ...
            H_dc = torch.stack(H_dc_list, dim=0)
            delta = torch.stack(delta_list, dim=0)
...
def create_dataloaders(
    ...
    if test_dir is not None:
        trainval_dataset = PowerFlowDCDeltaDataset(
        ...
        test_dataset = PowerFlowDCDeltaDataset(
    ...
    full_dataset = PowerFlowDCDeltaDataset(
        data_dir=str(train_dir),
        return_metadata=True,
        y_as_dense=y_as_dense,
        cache_metadata=cache_metadata,
