...
from powerflow_topology_utils_gpt0419fix import (
...
class NodeInputEncoder(nn.Module):
    """
    用 feature_visible_mask 在投影前剔除被遮挡特征的数值贡献，
    再在 token 空间加入可学习的 feature_mask_embed，避免：
    1) 在归一化空间直接置零所造成的“均值填充伪装成观测值”；
    2) 先 LayerNorm 再 mask 带来的特征间耦合污染。
    """
    ...
    def forward(
        ...
        if feature_visible_mask is None:
            ...
            if feature_visible_mask.shape != H.shape:
                raise ValueError(
                    f"feature_visible_mask shape {tuple(feature_visible_mask.shape)} != H shape {tuple(H.shape)}"
                )
...
class DynamicLocalMessagePassing(nn.Module):
    """
    只返回局部更新量 delta_x，由 block 外部执行 x = x + delta_x。
    这样局部子层和全局注意力/FFN 的残差形式保持一致。
    """
...
class HybridBlock(nn.Module):
    ...
    def forward(
        ...
        node_x = node_x + self.dropout(
            self.local_mp(self.local_norm(node_x), edge_x=edge_x, closed_mask=closed_mask, node_valid_mask=node_valid_mask)
        )
        node_x = node_x + self.dropout(
            self.global_attn(self.global_norm(node_x), node_valid_mask=node_valid_mask, attn_bias=attn_bias)
        )
